...
use xemm_rust::connector::hyperliquid::{
    OrderbookClient as HyperliquidClient, OrderbookConfig as HyperliquidConfig,
};
use xemm_rust::connector::pacifica::{
    OrderbookClient as PacificaClient, OrderbookConfig as PacificaConfig,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    loop {
        ...
        let (selected_direction, selected_price, selected_profit_bps) =
        ...
        if should_display {
            info!("═══════════════════════════════════════════════════");
            info!(
                "PACIFICA   | Bid: ${:.2} | Ask: ${:.2} | Mid: ${:.2} | Spread: ${:.3}",
                pac_bid, pac_ask, pac_mid, pac_spread
            );
            info!(
                "HYPERLIQUID| Bid: ${:.2} | Ask: ${:.2} | Mid: ${:.2} | Spread: ${:.3}",
                hl_bid,
                hl_ask,
                (hl_bid + hl_ask) / 2.0,
                hl_spread
            ...
            info!("───────────────────────────────────────────────────");
            info!(
                "BUY  Limit: ${:.2} | Distance: ${:.3} | Profit: {:.2} bps",
                buy_limit_rounded, buy_distance, buy_profit_bps
            );
            info!(
                "SELL Limit: ${:.2} | Distance: ${:.3} | Profit: {:.2} bps",
                sell_limit_rounded, sell_distance, sell_profit_bps
            );
            info!("───────────────────────────────────────────────────");
            info!(
                ">>> SELECTED: {} @ ${:.2} | Profit: {:.2} bps | Iter: {} <<<",
