...
async fn main() -> Result<()> {
    ...
    let symbol = if args.len() > 1 {
        ...
        println!(
            "{}",
            "Usage: cargo run --example fetch_recent_trades --release [SYMBOL]".yellow()
        );
    ...
    println!(
        "{}",
        format!("  Fetch {} Trades Test", symbol)
            .bright_cyan()
            .bold()
    );
    println!(
        "{}",
        "  Using BOT's Profit Calculation Logic"
            .bright_cyan()
            .bold()
    );
    println!(
    ...
    println!(
        ...
        "Finding most recent trade..."
    );
    println!();

    let (client_order_id, pacifica_timestamp) = match pacifica_trading
        .get_trade_history(Some(symbol), Some(5), None, None)
        .await
    {
        Ok(trades) => {
            if trades.is_empty() {
            ...
            let Some(client_order_id) = most_recent.client_order_id.clone() else {
                println!(
                    "{} Most recent Pacifica trade has no client_order_id; cannot match maker fills",
                    "WARN".yellow().bold()
                );
                return Ok(());
            };
            ...
            println!("{} Found most recent trade:", "OK".green().bold());
            println!("  Client Order ID: {}", client_order_display);
            println!(
                "  Price:           {}",
                format!("${}", most_recent.entry_price).cyan()
            );
            println!(
                "  Amount:          {} {}",
                most_recent.amount.bright_white(),
                symbol
            );
            println!(
                "  Fee:             {}",
                format!("${}", most_recent.fee).yellow()
            );
            ...
            (client_order_id, most_recent.created_at)
        }
        Err(e) => {
            println!(
    ...
    println!(
        "{} {}",
        "[TEST PACIFICA FETCHER]".magenta().bold(),
        "Using bot's fetch logic..."
    );
    println!();
        ...
        .await
    {
        ...
        if !all_matching.is_empty() {
            println!();
            println!(
                "  {} ALL fills with this client_order_id (before maker filter):",
                "DEBUG:".bright_black()
            );
        ...
        if !matching_trades.is_empty() {
            println!();
            println!(
                "  {} Pacifica fills breakdown (MAKER ONLY):",
                "DETAIL:".bright_blue()
            );
    ...
    match (
        pac_result.fill_price,
        pac_result.actual_fee,
        pac_result.total_size,
    ) {
        (Some(price), Some(fee), Some(size)) => {
            println!(
                "  Fill Price: {} (${:.6})",
                format!("${}", price).cyan().bold(),
                price
            );
            println!(
                "  Actual Fee: {} (${:.6})",
        ...
        (None, _, _) => {
            println!("  {} No trade data retrieved", "FAIL".red().bold());
        }
    }
    ...
    println!(
        "{} {}",
        "[TEST HYPERLIQUID FETCHER]".magenta().bold(),
        "Using bot's fetch logic..."
    );
    println!();
    ...
    let pac_time_str = pac_dt
        .map(|d| d.format("%Y-%m-%d %H:%M:%S UTC").to_string())
        .unwrap_or_else(|| pacifica_timestamp.to_string());

    println!(
        "  {} Retrieving Hyperliquid fills...",
        "INFO:".bright_blue()
    );
    println!(
        "  {} Pacifica trade timestamp: {} ({})",
        "INFO:".bright_blue(),
        pac_time_str,
        pacifica_timestamp
    );
    println!(
        "  {} Matching fills within +/-10 seconds of this time",
        "INFO:".bright_blue()
    );
    println!();
    ...
    let hl_result = match hyperliquid_trading.get_user_fills(&hl_wallet, true).await {
        Ok(all_fills) => {
            ...
            let matching_fills: Vec<_> = all_fills
                .iter()
                .filter(|f| {
                    f.coin == symbol
            ...
            if !matching_fills.is_empty() {
                println!(
                    "  {} Found {} matching Hyperliquid fill(s) within +/-10s",
                    "OK".green().bold(),
                    matching_fills.len()
                );
                ...
                println!(
                    "  {} No Hyperliquid fills found within +/-10s of Pacifica trade",
                    "WARN".yellow().bold()
                );

                // Show nearby fills for debugging
                let symbol_fills: Vec<_> = all_fills
                    .iter()
                ...
                if !symbol_fills.is_empty() {
                    println!(
                        "  {} Available {} fills (may be from different cycles):",
                        "DEBUG:".bright_black(),
                        symbol
                    );
                    for (i, fill) in symbol_fills.iter().enumerate() {
                        ...
                        let time_str = dt
                            ...
                            .unwrap_or_else(|| fill.time.to_string());
                        let time_diff =
                            (fill.time as i64 - pacifica_timestamp as i64) as f64 / 1000.0;
                        println!(
                            "  {}   Fill {}: {} @ ${} ({}, {:+.1}s from Pacifica)",
                            "DEBUG:".bright_black(),
                            i + 1,
                            fill.sz,
                            fill.px,
        ...
        Err(e) => {
            println!(
                "  {} Failed to retrieve Hyperliquid fills: {}",
                "FAIL".red().bold(),
                e
            );
            trade_fetcher::TradeFetchResult {
    ...
    match (
        hl_result.fill_price,
        hl_result.actual_fee,
        hl_result.total_size,
    ) {
        (Some(price), Some(fee), Some(size)) => {
            println!(
                "  Fill Price: {} (${:.6})",
        ...
        (None, _, _) => {
            println!("  {} No fill data retrieved", "FAIL".red().bold());
        }
    }
    ...
    if let (
        ...
        pac_result.total_notional,
        pac_result.actual_fee,
        pac_result.total_size,
        hl_result.total_notional,
        hl_result.actual_fee,
        hl_result.total_size,
    ) {
        println!(
        ...
        println!(
            "{} {}",
            "[PROFIT CALCULATION]".bright_green().bold(),
            "Combined hedge profit..."
        );
        println!();

        // Get the side from most recent Pacifica trade to determine direction
        let pac_side = pacifica_trading
            .get_trade_history(Some(symbol), Some(1), None, None)
            .await
            .ok()
        ...
        if size_diff_pct > 0.1 {
            println!("  {} Size mismatch detected!", "WARN".yellow().bold());
            println!(
            ...
            println!(
                "  {} This may indicate partial fills or matching issues",
                "WARN".yellow().bold()
            );
            println!();
        }
        ...
        println!(
            "  Size:            {} {}",
            pac_size.to_string().bright_white(),
            symbol
        );
        println!();
        println!(
            "  Pacifica Notional:  {}",
            format!("${:.6}", pac_notional).cyan()
        );
        println!(
            "  Hyperliquid Notional: {}",
            format!("${:.6}", hl_notional).cyan()
        );
        println!(
            "  Notional Diff:   {}",
        ...
        println!(
            "  Pacifica Fee:    {}",
            format!("-${:.6}", profit.pac_fee).yellow()
        );
        println!(
            "  Hyperliquid Fee: {}",
            format!("-${:.6}", profit.hl_fee).yellow()
        );
        ...
        println!(
            ...
            if profit.profit_bps >= 0.0 {
                format!("{:.2} bps", profit.profit_bps)
                    .bright_green()
                    .bold()
            } else {
        ...
        println!(
            "{} Cannot calculate combined profit - missing data from one or both exchanges",
            "WARN".yellow().bold()
        );
        println!();
    }
    ...
    println!(
        "{}",
        "OK This test uses the EXACT SAME code as the bot's profit calculation"
            .green()
            .bold()
    );
