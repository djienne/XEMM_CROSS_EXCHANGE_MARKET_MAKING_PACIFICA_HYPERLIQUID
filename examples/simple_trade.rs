...
async fn main() -> anyhow::Result<()> {
    ...
    let mid = (bid + ask) / 2.0;
    info!(
        "[PACIFICA] Market: Bid=${:.2} Ask=${:.2} Mid=${:.2}\n",
        bid, ask, mid
    );
    ...
    info!("\n[PACIFICA] OK Order placed!");
    info!("             Order ID: {}", order.order_id.unwrap_or(0));
    info!("             Client ID: {}", client_order_id);

    // Countdown 10 seconds
    info!("\n[PACIFICA] WAIT Waiting 10 seconds before cancelling...");
    for i in (1..=10).rev() {
    ...
    // Cancel order
    info!("\n[PACIFICA] delete  Cancelling order...");
    trading_client
        .cancel_order(symbol, &client_order_id)
        .await?;

    info!("\n[PACIFICA] OK Order cancelled successfully!");
    info!("\n═══════════════════════════════════════════════════");
