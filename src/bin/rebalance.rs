...
use xemm_rust::connector::pacifica::{
    trading::{OrderSide, PacificaCredentials, PacificaTrading},
...
async fn main() -> Result<()> {
    ...
    for i in 1..=max_retries {
        ...
        match rebalance_step(&config, &hl_trading, &pac_trading, &hl_price, &pac_price).await {
            Ok(is_balanced) => {
                if is_balanced {
                    info!("OK System is balanced. Exiting successfully.");
                    return Ok(());
                } else {
    ...
    warn!("WARN Max retries reached. System may still be imbalanced. Please check manually.");
    Ok(())
}
...
async fn rebalance_step(
    ...
    if net_delta.abs() < threshold {
        info!(
            "Net delta {:.4} is negligible. No action needed.",
            net_delta
        );
        return Ok(true);
    }
    ...
    if price_diff_pct > 0.05 {
        warn!(
            "Price deviation too high ({:.2}%). Aborting rebalance.",
    ...
    info!(
        "Imbalance detected: Net Delta = {:.4}. Rebalancing needed.",
        net_delta
    );
    ...
    // And |Change_HL| > min_hl_size.
        ...
        if is_reduce_pac(y) {
            ...
            if hl_enough && pac_enough {
                ...
                warn!(
                    "Dual leg strategy found but insufficient positions (HL: {}, Pac: {}).",
                    hl_enough, pac_enough
                );
            }
        } else {
    ...
    if let Some((size, is_buy)) = hl_trade {
        ...
        let safe_size = if hl_pos.abs() < size {
            hl_pos.abs()
        } else {
            size
        };
    ...
    if let Some((size, is_buy)) = pac_trade {
        ...
        if is_dual_leg && !hl_success {
            ...
            let safe_size = if pac_pos.abs() < size {
                pac_pos.abs()
            } else {
                size
            };
